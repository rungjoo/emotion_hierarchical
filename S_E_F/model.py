...
class EmoModel(nn.Module):
    def __init__(self, model_type):
        ...
        self.Ws = nn.Linear(self.model.config.hidden_size, self.sclsNum) # for classification
        self.We = nn.Linear(self.sclsNum, self.eclsNum) # for classification
        self.Wf = nn.Linear(self.eclsNum, self.fclsNum) # for classification
    ...
    def forward(self, batch_input_ids, batch_attention_mask):
        ...
        spred_outs = self.Ws(hidden_outs) # (B, L, C)
        epred_outs = self.We(spred_outs) # (B, L, C)
        fpred_outs = self.Wf(epred_outs) # (B, L, C)
